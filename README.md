...
- **Fake-ads easter egg:** typing a secret sequence in a room shows a fake ad
  overlay to everyone else. The repo ships no ad images (the overlay stays
  invisible) — drop your own PNGs into `frontend/src/ads/` (see the README
  there) to arm it.
...
- The catalog leans German/international (it was curated for a German friend
  group) — fork the seed lists to match your crowd; that's the point.

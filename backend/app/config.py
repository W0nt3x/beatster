...
Design: ONE base directory (``HITSTER_DATA_DIR``) holds all mutable state
(catalog cache, community songs, stats DB, room snapshots), so a fresh or
self-hosted instance needs at most one env var — or none at all, the default
matches the historical layout (``~/.cache/hitster``). Every path can still be
overridden individually (that's how a production service unit pins its paths
to a system location), and the individual override wins.

Every knob reads ``BEATSTER_<NAME>`` first (the documented, public spelling)
and falls back to the legacy ``HITSTER_<NAME>`` alias — the private
production deployment and old local setups predate the rename and keep
working untouched.
...
def _env(name: str) -> str | None:
    """BEATSTER_<name> wins; HITSTER_<name> is the honored legacy alias."""
    return os.environ.get(f"BEATSTER_{name}") or os.environ.get(f"HITSTER_{name}")
...
DATA_DIR = _env_path("DATA_DIR", os.path.expanduser("~/.cache/hitster"))
...
DB_PATH = _env_path("DB", os.path.join(_CACHE_DIR, "hitster.db"))
